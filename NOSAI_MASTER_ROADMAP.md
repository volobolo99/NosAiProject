...
- Still not verified, and not claimed: gameplay observation remains `UNKNOWN` (`gameplay_provider_not_available`) -- ADR-0012/ADR-0014 leave the provider unbuilt, so the channel carries a snapshot with no gameplay in it. Wi-Fi transport was not exercised; only USB was.
- Open finding, not fixed: the phone reports `keystore_private_key_unavailable` and keeps its device key as a file inside the app rather than in the Android Keystore. ADR-0010 puts key custody in the platform key store, so this device is running a weaker custody than the ADR describes. Recorded here rather than silently accepted.
