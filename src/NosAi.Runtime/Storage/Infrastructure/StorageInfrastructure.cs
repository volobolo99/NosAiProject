...
namespace NosAi.Storage.Infrastructure
{
    ...
    public static class Program
    {
        ...
        {
            Console.Title = "NosAi Storage & Infrastructure Engine (1.0 Beta)";

            if (args.Length > 0 && args[0].Equals("--test", StringComparison.OrdinalIgnoreCase))
            {
                bool success = await StorageInfrastructureTestRunner.RunAllTestsAsync();
                return success ? 0 : 1;
            }

            Console.WriteLine("Inizializzazione NosAi Storage & Infrastructure Engine...");
            Console.WriteLine("Esecuzione della suite di test per volume NOSAI-SSD, SQLite WAL e Backup...");

            bool passed = await StorageInfrastructureTestRunner.RunAllTestsAsync();
            return passed ? 0 : 1;
        }
    }
