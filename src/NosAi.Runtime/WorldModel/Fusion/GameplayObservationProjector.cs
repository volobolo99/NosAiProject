...
/// <list type="bullet">
/// <item><description><see cref="WorldModelSnapshot.Mobs"/>/<see cref="WorldModelSnapshot.Npcs"/> stay empty here, but the reason is narrower than "no classification exists": the wire's entity type 3 really is monster and NPC together (<c>NosAi.Runtime.Autonomy.SelectableEntity.Vnum</c>'s own remark), but <c>NosAi.Runtime.Autonomy.TargetEstablishment.Assess</c> already resolves exactly this ambiguity for combat authorization, via <c>GameReferenceDatabase.Exists("monster", vnum)</c>, and is already wired into the live targeting path (<c>Gate3Runtime</c>). What is missing is only wiring the same lookup <i>here</i>, to turn <see cref="GameplayObservation.Entities"/>'s <c>SelectableEntity</c> list into real <see cref="Mob"/> records for this projection specifically -- a mechanical gap, not a classification one. The health such a <see cref="Mob"/> would carry no longer needs a design decision either: <see cref="Resource.FromObservedFraction"/> represents <c>SelectableEntity.HpRatio</c> honestly, as a real fraction over two Unknown bounds. Where the absolutes exist they should be preferred over the fraction -- <c>NosTaleWorldProtocolDecoder.DecodeOtherVitals</c> reads a non-player entity's absolute current and maximum HP off the <c>st</c> packet and divides them away only because <c>EntitySighting</c> has nowhere to put them, so widening that record is the first of the two steps.</description></item>
