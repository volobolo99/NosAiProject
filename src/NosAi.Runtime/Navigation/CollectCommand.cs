...
public static class CollectCommand
{
    ...
    private sealed class LiveScope : IDisposable
    {
        ...
        /// <summary>
        /// Opens a real, scoped WinDivert capture of the client process's own
        /// game connection and composes the live provider over it.
        /// </summary>
        /// <remarks>
        /// The composition mirrors <see cref="Gate1ObservationChannel.FromPackets"/>
        /// (src/NosAi.Runtime/Gate1/Gate1ObservationChannel.cs lines ~150-180):
        /// <c>ReassembledObservationSource.ForNosTaleWorld</c> over a packet
        /// source, decoded by <see cref="NosTaleWorldProtocolDecoder"/> into a
        /// <see cref="NetworkWorldFeed"/> and published by
        /// <see cref="NetworkGameplayProvider"/>. The packet source is
        /// <see cref="WinDivertPacketSource"/> opened on the client's single TCP
        /// game connection, whose address and port are read from the OS
        /// connection table by <see cref="ClientNetworkObserver"/> -- the same
        /// discipline <see cref="WireRecorder"/> and
        /// <see cref="Gate1ObservationChannel.TryOpenLive"/> use (a hard-coded
        /// server address would keep capturing a host the client had already
        /// stopped talking to). The <see cref="RealClientConnector"/> half is
        ...
        /// <see cref="LiveObservationGateway.Capture"/>, never its transport
        /// (same construction LiveObservationGatewayTests uses). Disposal
        /// order: the capture is released first through the observation source
        /// that owns it, then the connector (which disposes its channel), then
        /// the auth key.
        /// </remarks>
