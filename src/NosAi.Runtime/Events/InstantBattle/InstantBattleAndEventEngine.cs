...
namespace NosAi.Events.InstantBattle
{
    ...
    public readonly record struct Position2D(int X, int Y) { public double DistanceTo(Position2D other) { int dx = X - other.X; int dy = Y - other.Y; return Math.Sqrt(dx * dx + dy * dy); } }
    public sealed record GroundDropItem(long DropId, string Name, int ItemId, int Quantity, Position2D Position, long EstimatedValueGold, bool IsHighValueMaterial, DateTime DroppedAtUtc);
    public sealed record EventActionIntent(Guid IntentId, string ActionKind, long TargetEntityId, Position2D TargetPosition, float PriorityScore, string TacticalRationale);
    ...
    public sealed class InstantBattleWaveSolver
    {
        ...
        private readonly Position2D _arenaSafeCorner = new(15, 15);
        private readonly object _lock = new();
        public InstantBattleWaveStage CurrentWave { get { lock (_lock) return _currentWave; } }
        public Position2D ArenaSafeCorner => _arenaSafeCorner;
    ...
    public sealed class InstantBattleAndEventOrchestrator
    {
        ...
        private readonly List<GroundDropItem> _activeGroundDrops = new(); private readonly List<Position2D> _activeMobPositions = new();
