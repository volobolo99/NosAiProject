...
- Aggiunto `tests/NosAi.ControlPanel.UiTests` (C-007): un driver (`UiAutopilot`) che pilota il Control Panel come farebbe un operatore reale — click fisico via `Win32InputBackend`, cattura schermo via `DxgiDesktopDuplicationSource`, localizzazione dei controlli via UI Automation — per test end-to-end autonomi, senza intervento manuale. Riusa i due backend già pubblici in `NosAi.Runtime`, non ne introduce di nuovi. Primo scenario verificato dal vivo (`NavigationUiTests`): avvio di `NosAi.ControlPanel.exe`, click su "Client NosTale", verifica del cambio di titolo pagina. Il primo tentativo è fallito perché il click fisico non portava la finestra in primo piano (`SendInput` colpiva qualunque finestra fosse sopra sullo schermo); corretto con `SetForegroundWindow` prima di ogni click. Categoria xunit `UiAutopilot`: esclusa di default da `scripts/validate.ps1` e dai workflow CI, perché sposta il mouse fisico reale — si lancia a mano con `dotnet test --filter Category=UiAutopilot`. Limite noto: assume la finestra sul monitor primario. Copre solo il Control Panel; l'aggancio al gameplay reale resta dietro il gate di `ADR-0003` e non è nello scope di questo contratto.
