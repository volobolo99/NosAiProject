...
| C-310 | Optimization: meta' decisionale del dispatch di StrategicGoalKind.Optimization (selezione candidato + verifica in borsa, senza il click) | TEST_VERIFIED |
  ...
  - note: Ambito ridotto in corsa (amendment_2026-09-12 dentro contracts/equip-command-executor-wiring-016.json): il contratto originale prevedeva anche il click reale via EquipExecutor.Equip, ma verificare l'esito richiede un WornEquipment fresco con EntityId, che oggi si ottiene solo aprendo un secondo GameTrafficObserver sullo stesso endpoint di rete gia' catturato una volta per ciclo da RunWindowsCore -- una decisione architetturale (condividere o duplicare la cattura) non progettata qui e non improvvisata. EquipCommand.cs, EquipExecutor.cs e BagPanelRoiCalibration.cs restano scheletri (NotImplementedException): il click vero e' rimandato a un contratto di follow-up. Nessuna calibrazione data/perception/bag-panel-roi.calibration esiste ancora: richiede l'operatore al client reale, come gia' fatto per T-12 il 2026-09-06.
