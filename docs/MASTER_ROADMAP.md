...
| C-402 | Fuzzing sui pacchetti corrotti | TEST_VERIFIED |
  - updated: 2026-09-11
  - metrics: Harness di fuzzing deterministico creato il 2026-09-11 (contratto contracts/wire-protocol-fuzz-harness-011.json): src/NosAi.Runtime/Testing/WireProtocolFuzzTestRunner.cs, sei controlli su WireHeader.TryRead (src/NosAi.Protocol/WireProtocol.cs, non modificato) -- lunghezze troncate, round-trip su tutti i 256 valori di MessageType, round-trip sui valori limite di PayloadLength/SequenceNumber, 34 varianti di magic sbagliato, 255 versioni sbagliate, 50000 buffer generati/mutati con seed fisso 1337. Un difetto reale trovato dal collaudo (non dalla preflight, che aveva approvato): la logica di tre controlli era invertita (testava TryRead()==false come fallimento invece di successo), diagnosticato e corretto da deep_reasoner_solve_crash (DeepSeek) sull'evidenza reale di esecuzione, non per ispezione. Collegato alla tabella delle suite (--wire-fuzz-test) e a un Fact xUnit dedicato (WireProtocolFuzzTests.cs). Verificato: dotnet build 0 errori/0 avvisi, --wire-fuzz-test 6/6 PASS, dotnet test sulla suite .NET intera 2832/2841 (9 ignorati, 0 falliti, +1 rispetto alla baseline per il nuovo Fact).
