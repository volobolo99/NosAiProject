...
| 5 | Exploration | `Present` | `AP-04_A1_STATUS.md`: A1+A3 `Present`. | A2+A4 (`--scout`) specificato per DeepSeek, non ancora eseguito. |
| 6 | Navigation | `Present` | Il cammino cella-per-cella (`PathWalkController`/`WalkCommand`) è reale e Gate-1-verificato come **meccanismo**, ma la catena "il World Model sceglie dove andare → si cammina davvero" (`--scout`) non è ancora Integrated end-to-end. | Stesso di Exploration: `ScoutCommand` non ancora scritto. |
