...
- ~~**Finestre di dialogo**: nessun codice di lettura esiste~~ — **Presenza/assenza** di un pannello aperto costruita in questo passaggio (A1+A3, Claude), vedi §12. ~~**contenuto testuale** bloccato da OCR/ML~~ — **corretto il 2026-09-08**: la premessa regge (nessun opcode del filo porta dialogo o testo di missione), la conseguenza no. Il testo sta nei file del client ed è importato dal 2026-09-07 (Q-129): `quest` e `npctalk` fra le `ReferenceImporter.TextOnlyTables` (`ReferenceImporter.cs:134-135`), 3 639 testi di missione e 22 358 battute di NPC. Resta aperto il collegamento id sul filo → chiave di tabella, che non è un problema di OCR.
