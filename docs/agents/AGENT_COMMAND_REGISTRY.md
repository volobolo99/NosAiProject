...
**v1.1:** every A2/A4 "Cursor" role is now "DeepSeek" — DeepSeek is active and takes over Cursor's role entirely across every phase (user instruction, 2026-09-05). If Cursor becomes available again, its role is redistributed on explicit request; it does not automatically reclaim these slots.
...
- A2 DeepSeek: `src/NosAi.Runtime/` integration/bootstrap/runtime adapters and direct runtime tests.
- A3 Claude: Core duplicate/compatibility audit; only files explicitly assigned after inspection may be written.
- A4 DeepSeek: `src/NosAi.ControlPanel/` startup, runtime status and cognitive-dashboard integration.
...
- A2 DeepSeek: perception-to-world-model adapters and sensor normalization in the existing perception/runtime locations.
- A3 Claude: temporal belief, prediction and derived-state algorithms in dedicated WorldModel/Planning files.
- A4 DeepSeek: runtime wiring from existing observation snapshots into the World Model; no contract redesign.
...
- A2 DeepSeek: observation/evidence adapters.
- A3 Claude: path planning, replanning and cost/risk algorithms.
- A4 DeepSeek: runtime navigation loop and movement verification.
...
- A2 DeepSeek: client-observable inventory/character observation adapters.
- A3 Claude: loadout/equipment planning and resource constraints.
- A4 DeepSeek: runtime character-control integration and verification.
- A5 Claude: tests and docs.
- A6: integration.

### AP-08 — Strategic Autonomy
- A1 Claude: strategic contracts and goals.
- A2 DeepSeek: attention/state aggregation adapters.
- A3 Claude: strategic planner, utility/risk and long-horizon selection.
- A4 DeepSeek: orchestrator/runtime lifecycle integration.
...
- A2 DeepSeek: persistence/runtime adapters.
- A3 Claude: outcome learning, simulation and conservative adaptation.
- A4 DeepSeek: runtime memory bridge and observability integration.
...
- A2 DeepSeek: certification observation adapters.
- A3 Claude: certification runner and deterministic acceptance logic.
- A4 DeepSeek: runtime/test-center integration.
