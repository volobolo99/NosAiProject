...
## Purpose
This protocol defines the mandatory stabilization pass before autonomous multi-agent development proceeds.
...
A task is complete only when affected files are complete, compile, tests pass, documentation is updated, and evidence is recorded. No TODO/FIXME/stub/pseudocode/placeholder is accepted in delivered implementation.

## Parallelism Rule
Agents may work concurrently only on disjoint file ownership. Shared contracts, project files and integration points are serialized through the Integration Agent. No agent may overwrite another agent's work.
...
## Required Handoff
Every agent reports files changed, implementation summary, tests, build result, known limitations, integration dependencies and exact commit SHA.
