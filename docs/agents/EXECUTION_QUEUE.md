...
- **DeepSeek: ATTIVO dal 2026-09-05, prende il ruolo di Cursor INTERAMENTE** (istruzione esplicita dell'utente). Ogni riga "A2 Cursor"/"A4 Cursor" in `docs/agents/AGENT_COMMAND_REGISTRY.md`, per qualunque fase, è ora "A2 DeepSeek"/"A4 DeepSeek" — vedi quel file v1.1. Da questo momento questi task vanno assegnati a DeepSeek, non più eseguiti da Claude al suo posto: Claude scrive/aggiorna il file di comando dettagliato, l'utente lo apre e lo passa a DeepSeek. Il backlog completo, con la regola non negoziabile sui file consegnati, è in `docs/agents/DEEPSEEK_TASKS.md`. I task già completati da Claude "al posto di Cursor/DeepSeek" prima di questa attivazione (marcati così in coda, es. Q-002, Q-004, Q-008, Q-014→Q-018, Q-020) restano `DONE`/`Integrated` così come sono — non vengono rifatti. L'unica eccezione: AP-03/A4 (Q-022) era già stato assegnato a un agente Claude in background nel momento in cui DeepSeek è stato attivato — non è stato interrotto a metà lavoro per non sprecarlo; da AP-04 in poi ogni nuovo task A2/A4 va a DeepSeek senza eccezioni.
- **Claude**: scrive i comandi dettagliati per ogni task man mano che diventa il prossimo in coda; esegue i task assegnati a "Claude" (A1/A3/A5); per i task DeepSeek fa da integratore (A6) — collegamenti tra moduli, deduplicazione, risoluzione conflitti, build/test finale, commit.
