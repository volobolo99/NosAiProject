...
| Bersaglio (`HasTarget`) | **stabilito in memoria**, non ancora acceso su sessione | l'oracolo comportamentale ha concluso il 2 settembre: `manager+0x44`, sentinella 0, sei selezioni, due deselezioni, un riavvio superato. Il valore e' un **puntatore** all'oggetto del bersaglio, non un id — e per `HasTarget` va meglio: non-zero e' bersaglio, zero e' nessuno, `DERIVED` perche' il client non pubblica un booleano. Lo zero e' una risposta, non una catena rotta: trattarlo da errore fonderebbe due dei tre stati di `ADR-0018` |
| Proiezione mappa→schermo | **calibrata il 2026-09-07** | il modello era affine e la mappa e' prospettica: la casella vale piu' pixel in basso che in alto, misurato su due sessioni indipendenti. Passo 38,14 x 16,83 px per casella, personaggio a (516, 445) di 1024x768, 19 campioni su 20, versionata in `data/perception/`. Vedi `docs/TEST_RIMANDATI.md` **T-15** |
