...
| `nosai.network` | 650 | `phone` | — | `tests/network/test_wire_protocol.py`, `test_worldstate_delta.py`, `test_session_cipher.py`, `test_crypto_auth.py` | `src/NosAi.Protocol` (framing NOSA) | INCERTO |
...
| `nosai.phone` | — | — | `docs/GATE1_CHECKLIST.md`, `docs/INTEGRAZIONE_RSA_SESSION_AUTH.md`, `docs/adr/ADR-0007`, `ADR-0008`, `ADR-0010` | — | `src/NosAi.GuardClient` (client sul filo, non ADB) | RIMOSSO (`fa908f2`); i sei test che lo importavano sono stati rimossi il 2026-09-07, rompevano la raccolta di pytest in CI |
