...
| T-08 | Gate 3 — decisione sul client vivo | Il ciclo di decisione ora esiste come processo: `NosAi.Runtime.dll --decide-cycles <n>` lo avvia dentro il Gate 1 e pianifica sullo snapshot canonico. **Il nome corretto e' questo**: fino al 2026-09-08 questa riga diceva `--decide`, che non e' mai esistito -- `Program.cs` conosce solo `--decide-cycles` e `--decide-replay`, e un incarico scritto su quel nome avrebbe prodotto un comando inesistente. Riscontro offline ripetibile: `NosAi.Runtime.dll --decide-replay data/nostale_combat.noscap` produce decisioni su HP 7218/7305 e 7305/7305 letti dalla cattura reale, entrambe `NoCandidate` (personaggio sano, bersaglio sconosciuto). **Resta da fare:** avviare con `--observe-game` su una sessione in corso e vedere il ciclo passare da `NoWorldState` a una decisione su letture `LIVE`. Nulla può agire: la politica di sicurezza tiene l'input disabilitato e l'effector è `DisabledActionEffector`, quindi l'esito atteso a valle del Safety Gate è `ExecutionDisabled`. | sì |
