...
| T-08 | Gate 3 — decisione sul client vivo | Il ciclo di decisione ora esiste come processo: `NosAi.Runtime.dll --decide` lo avvia dentro il Gate 1 e pianifica sullo snapshot canonico. **Nota per chi lo cerca nel codice:** `--decide`, `--observe-game` e `--decide-interval-ms` non sono comandi di `Program.cs` ma opzioni dell'host Gate 1, lette da `Gate1HostOptions` (`:153`, `:218`, `:154`); cercarle nel dispatch di `Program.cs` non le trova, e il 2026-09-08 questa riga e' stata corretta per errore proprio cosi', poi rimessa. Verificato per esecuzione: `--decide --observe-game <ip>:<porta> --decide-interval-ms 5000` fa registrare `Game observation attached. endpoint=... source=LIVE` e avvia il bootstrap Gate 1. Riscontro offline ripetibile: `NosAi.Runtime.dll --decide-replay data/nostale_combat.noscap` produce decisioni su HP 7218/7305 e 7305/7305 letti dalla cattura reale, entrambe `NoCandidate` (personaggio sano, bersaglio sconosciuto). **Resta da fare:** avviare con `--observe-game` su una sessione in corso e vedere il ciclo passare da `NoWorldState` a una decisione su letture `LIVE`. Nulla può agire: la politica di sicurezza tiene l'input disabilitato e l'effector è `DisabledActionEffector`, quindi l'esito atteso a valle del Safety Gate è `ExecutionDisabled`. **Evidenza parziale raccolta il 2026-09-12** sul client vivo (stesso consenso operatore di T-05): `--decide --observe-game 79.110.84.175:4005 --decide-interval-ms 5000` fa partire il ciclo (`Gate 3 decision loop started`) con `acting=False` e `Actuation authority verdict taken. actuating=False reason=authority_live_input_not_armed` — l'esito di sicurezza atteso, confermato. **Non ancora osservato:** la transizione esplicita da `NoWorldState` a una decisione: in due finestre di cattura (~25 s e ~40 s) non c'erano mostri/entità nei paraggi del personaggio — coerente con `--combat-report` sulla stessa sessione (`mobs: 0 observed`) e con `/api/gate1` che segnala per questo percorso di attach `"Gameplay fields (HP, map, entities) remain UNKNOWN: no gameplay provider is bound"`. Serve rieseguire con il personaggio vicino a un mostro reale. **Chiuso il 2026-09-12, rieseguito con il personaggio vicino a un mostro:** `--combat-report` sulla stessa sessione conferma `mobs: 1 observed, 0 known hostile, 1 known alive, 1 positioned` (mostro visto e vivo, non ancora ostile perché non ha ancora colpito il personaggio). Riavviato `--decide --observe-game 79.110.84.175:4005 --decide-interval-ms 5000` e interrogato `/api/gate1` — la stessa funzione (`_snapshot.Capture`) che `Gate1SnapshotWorldStateSource.ReadAsync` legge (`Gate3WorldState.cs:375-410`): `client.attached=true` e `client.gameplayBaseline` popolato con `hp=7693/maxHp=7693/mp=1787` tutti `LIVE`, `entitiesInView=58`, 118 entità in cache di cui 112 classificate `Monster` dal catalogo di riferimento. **Il binario live non stampa né espone l'esito per ciclo del loop Gate3** (verificato in `Gate3DecisionLoop.cs` e `Program.cs`: nessun sink di telemetria è cablato sul percorso `--decide`, nessuna riga di log per ciclo, e `DecisionTelemetryWriter` non viene mai costruito in `Gate1BootstrapHost.cs`), quindi la transizione si legge dal codice della sorgente invece che da una riga di console: con `HasVitals` vero (`hp`+`maxHp`+`mp` tutti presenti), `Gate1SnapshotWorldStateSource.ReadAsync` ritorna `Gate3WorldState.FromObservation(gameplay)` invece di `Unobserved(...)` — la sola condizione che produce `NoWorldState` (client non attaccato, o `Client.Gameplay` nullo) non vale più. `hasTarget` resta `False` (nessun bersaglio ancora selezionato/colpito), quindi l'esito del ciclo resta `NoCandidate`, coerente con `--decide-replay`: non un guasto, l'assenza prevista di un candidato senza ostilità stabilita. | no |
...
| T-14 | Una cattura di rete che cominci **prima** del login | Tutte e cinque le catture esistenti sono state prese a client gia' in gioco, quindi non contengono i pacchetti di caricamento personaggio. Censite il 2026-09-07 con `--world-replay`: 27 726 messaggi inbound complessivi, **zero `ski`** (la lista abilita'), e `sc`/`equip`/`inv` solo parzialmente in `equip_test.noscap`. **Cosa fare, dal 2026-09-08 senza digitare nulla:** Pannello di controllo -> **Rete** -> riquadro **«T-14 — Registra dal login (prima del collegamento)»**, bottone «Registra dal login». Il gesto e' scritto nel riquadro: 1) chiudi il client NosTale, 2) premi, 3) apri il client, accedi ed entra in gioco, 4) lascia correre qualche secondo. La registrazione comincia e finisce da sola, e a fine corsa il pannello mostra file, pacchetti e quanto ha atteso prima di agganciare. Un solo file basta. **Limite dichiarato dal riquadro stesso:** la cattura comincia quando aggancia la connessione del client, quindi i pacchetti scambiati prima dell'aggancio non sono catturati -- e' «dalla connessione in poi», non «dall'accensione in poi». Per i pacchetti di caricamento personaggio, che arrivano dopo il login, e' sufficiente. La vecchia forma `--record-wire <ip>:<port> <file.noscap>` resta valida e invariata. **Perche' serve:** e' l'unica cosa che sblocca `docs/agents/phases/AP-05/AP-05_A2A4_DEEPSEEK_player_skill_list.md` parte 2, cioe' il canale di osservazione a monte dei due difetti gia' corretti in `5e9bd93` e `a39197e`. Senza, un decoder per `ski` sarebbe un parser mai eseguito su un byte reale, che la regola sulle fonti esterne di `CLAUDE.md` vieta. Nessun altro esperimento e' necessario: il censimento ha gia' escluso che il pacchetto sia nascosto nelle registrazioni esistenti. | si' |
