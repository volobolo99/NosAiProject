...
- molte aree oltre il Gate 1 sono **Present** o **Integrated**. La prima misura
  con evidenza su client reale è arrivata il **2026-09-07**: la proiezione
  schermo → mappa è calibrata (`data/perception/screen-projection.calibration`,
  passo 38,14 × 16,83 px per casella, personaggio disegnato a (516, 445) di
  1024×768 a 120 DPI, 19 campioni su 20, il runtime la dichiara *usable*). È la
  misura a essere verificata, non l'area attorno: quel che ci poggia sopra —
  `InputActionEffector.TryProject` e i clic che ne derivano — resta
  **Integrated** finché un operatore non osserva un clic andare dove doveva.
  Il percorso e i due modelli scartati stanno in `docs/TEST_RIMANDATI.md` **T-15**;
...
2. la sessione resta una sola, ma l'ammissione ora è concorrente (ADR-0011): uno squatter non tiene più fuori il peer legittimo, nemmeno parlando e poi fermandosi: la scadenza si misura dall'accettazione (`Gate1Runtime.cs:304`) e un reaper la applica (`:721`). Verificato, `GuardAdmissionTests` 8 su 8;
3. le chiavi di identità non sono più in chiaro (ADR-0010): DPAPI sul PC, Android Keystore sul telefono. Resta che DPAPI lega all'account Windows e non a un TPM; la prova del Keystore su dispositivo è **fuori scope** (ADR-0025).
...
- Autorità di attuazione legata alla sessione (`SessionActuationAuthority`, P3): prima di offrire al livello decisionale la capacità di agire, il runtime confronta il proprio livello di integrità con quello del client e poi lo **prova** — un movimento di puntatore di pochi pixel dentro la finestra del client mentre questa è in primo piano, riletto e riportato dov'era. Il caso che rende necessaria la prova è che `SendInput` da un processo a integrità media verso un client a integrità alta **fallisce senza dirlo**: né il valore di ritorno né l'ultimo errore lo segnalano, e un ciclo di ritentativi legge quel silenzio come « il gioco non risponde » e gira per sempre. Due rifiuti sono **terminali e non vengono più riprovati** — runtime sotto il client, e puntatore che non arriva dove è stato mandato — perché richiedere di nuovo non li cambia e farebbe sobbalzare il puntatore dell'operatore a ogni ciclo; se ne esce con una sessione nuova o con un `Reset` dell'operatore. Una sessione non attuante **non espone alcuna capacità di attuazione** (`InputActionEffector.UnavailableReason`) invece di esporne una che fallisce all'uso, e l'osservazione continua intatta. Verdetto valido 60 s e ripreso al ritorno in primo piano. Superficie `S1` (`X-P3`): `--input-authority [--watch n]`, campi additivi sullo snapshot (`sessionActuating`, `sessionAuthorityReason`, `sessionAuthorityTerminal`, `runtimeIntegrity`, `clientIntegrity`), riga di stato nel Control Panel e nella dashboard — il pannello chiede, non decide, e su un verdetto terminale non offre un ritentativo. Il ciclo decisionale chiama `EnsureVerified()` prima del piano; non esiste un osservatore del primo piano, quindi l'unico innesco di ripresa è `--watch`. Evidenza **locale** (suite runtime): il giro con il client elevato non è ancora stato fatto.
...
- Superficie di resilienza (`S2` / `X-P8`): una transizione del breaker verso `Halted` scrive **un** dump in `data/halt-*.json` (gitignored) con stato precedente e nuovo, finestra dei fallimenti, ultimo rifiuto del commit point, ultimo verdetto di autorità di sessione, ultimo esito per stage. Non è un campionamento a intervalli. `--halt` (CLI verso un runtime già in ascolto) e il comando HALT del Control Panel disarmano gli interruttori e poi abortiscono lo scope aperto, solo `SecurityPrincipal.Operator`, e un secondo halt non è un errore. Non chiude la sessione Guard: quello resta EMERGENCY STOP. Lo snapshot Gate 1 porta `resilience` (stato, fallimenti in finestra, attesa al prossimo tentativo, budget); un controller assente è `UNKNOWN` con `recovery_controller_not_configured`, mai uno zero quieto. Il pannello chiama `EventLogDiagnostics` e un registro con gap si vede come **INCOMPLETO**. Proprietà di sola lettura aggiunte sul controller: `FailureWindow`, `BaseCooldown`, `MaxCooldown`. Soglie e stati del breaker **non** sono cambiati. Livello: `Done`, non `Verified`.
...
Le voci che richiedono uno smartphone sono **fuori scope** dalla roadmap corrente (ADR-0025, 2026-09-05): restano elencate come evidenza storica, non come lavoro pendente. Non vanno riaperte.
...
- Custodia delle chiavi di identità (`ADR-0010`): implementata su entrambi i lati. Sul PC l'identità è avvolta con DPAPI in `data/runtime_identity.dpapi` e la migrazione dal PEM in chiaro **conserva la stessa chiave**, quindi nessun telefono già abbinato deve rifare l'abbinamento — verificato sull'identità reale di questa macchina. Sul telefono la chiave è generata **dentro** l'Android Keystore. **Resta Partial per la sola parte PC**: DPAPI lega la chiave all'account Windows e non a un TPM. Il giro Keystore su dispositivo fisico è **fuori scope** (ADR-0025): non è lavoro pendente; dove il Keystore manca l'app ripiega su file e **lo dichiara** invece di sembrare protetta.
- Sessioni Guard concorrenti (`ADR-0011`): il canale serve **una sola sessione**, ma ammette fino a quattro candidati che fanno l'handshake in parallelo, e vince chi si autentica per primo. Lo stato dell'handshake è ora per connessione, non più campi condivisi. Uno squatter silenzioso non tiene più fuori il telefono: viene sfrattato per primo e il telefono si autentica accanto a lui. **Chiuso il 2026-09-08**: la scadenza di ammissione si misura da `AcceptedAtUtc` (`Gate1Runtime.cs:304`, `AuthenticationDeadline` 1500 ms) e un reaper periodico (`Gate1Runtime.cs:721`) chiude ogni candidato che non si è autenticato entro quel termine, quindi colpisce anche chi ha parlato e poi ha smesso, senza attendere che arrivi una nuova connessione. Verificato da `GuardAdmissionTests`: `ASilentPeerIsDroppedByTheAdmissionDeadline_NotTheHeartbeatOne`, `SquattersFillingEverySlotDoNotKeepThePairedPhoneOut` e `TheSlotComesFreeForTheNextPeerAfterASquatterIsEvicted`, verdi nella suite del 2026-09-08 (2784 superati su 2793).
- Cifratura autenticata (AEAD) del payload di sessione: implementata e verificata in locale (wire v3, ADR-0009), **non ancora provata sul dispositivo fisico**, e non lo sarà: la prova su telefono è **fuori scope** (ADR-0025). Resta evidenza storica che un APK v2 viene rifiutato all'header, come previsto.
