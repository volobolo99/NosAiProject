...
Il prototipo Python descritto sotto (`app/simulation/tactical.py`) non esiste
piu': la linea Python e' stata rimossa quando il C# ha coperto le stesse
funzioni (`3f0934a`). L'implementazione corrente e'
`src/NosAi.Runtime/Tactical/` (`Simulation.cs`, `TacticalPlanner.cs`,
`TacticalRanking.cs`). L'elenco che segue resta la specifica delle capacita'
attese dal simulatore, non la descrizione di un file esistente.
